...
module Hpffp.ParsingLogFileSpec where
...
-- We really should drop sleep as an activity. Sleep should be
...
spec = do
    describe "Parsing a Log file" $ do
        it "can work with it" $ do
            let result = countDays $ extractDates parsedLog
            result `shouldBe` 1
