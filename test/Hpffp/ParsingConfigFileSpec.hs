...
module Hpffp.ParsingConfigFileSpec where
...
parseBracketPair p =
    char '[' *> p <* char ']'
    -- there operators mean the brackets
...
spec = do
    describe "Parsing a config " $ do
        it "ignores the 2nd key/value pair if linebreak not skipped" $ do
            let spa' = some parseAssignment'
            let s = "key=value\nblah=123"
            let (Success x) = parseString spa' mempty s
            x `shouldBe` [("key","value")] -- it ignores 'blah=123'
        it "picks up both key/value pairs" $ do
            let d = "key=value\n\ntest=data"
            let spa = some parseAssignment
            let (Success x) = parseString spa mempty d
            x `shouldBe` [("key","value"),("test","data")]
